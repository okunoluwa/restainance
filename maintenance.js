// my-requests.js - COMPLETE WORKING VERSION WITH FULL NAME DISPLAY
...
function getCurrentStudentInfo() {
    ...
    let studentNumber = localStorage.getItem('currentStudentNumber') ||
                        localStorage.getItem('reportStudentNumber') ||
                        localStorage.getItem('studentNumber');
    let studentName = localStorage.getItem('studentName');
    
    // If no student name, try to get from user object
    if (!studentName) {
        ...
        if (userData) {
            try {
                ...
                if (user && user.studentNumber && !studentNumber) {
...
function exportToPDF() {
    ...
    const { studentNumber, studentName } = getCurrentStudentInfo();
    if (!studentNumber) return;
    const myRequests = allRequests.filter(req => req.studentNumber === studentNumber);
    
    let html = `<html><head><title>My Requests</title></head><body>
        <h1>My Maintenance Requests</h1>
        <p>Student Name: ${escapeHtml(studentName || studentNumber)}</p>
        <p>Student Number: ${escapeHtml(studentNumber)}</p>
    ...
    link.download = `my-requests-${studentNumber}.pdf`;
    link.click();
    alert('✅ PDF exported!');
}
...
function renderRequests() {
    ...
    const { studentNumber, studentName } = getCurrentStudentInfo();
    console.log('👤 Current student number:', studentNumber);
    ...
    const myRequests = allRequests.filter(req => req.studentNumber === studentNumber);
    console.log(`🎯 Found ${myRequests.length} requests for student ${studentNumber}`);
    ...
    myRequests.sort((a, b) => b.id - a.id).forEach(request => {
        ...
        card.innerHTML = `
            ...
            <p><i class="fas fa-user"></i> <strong>Student:</strong> ${escapeHtml(request.studentName || request.studentNumber)}</p>
