...
function getCurrentStaffName() {
    ...
    if (userData) {
        try {
            ...
            if (user && user.role === 'staff' && user.fullname) {
                currentStaffName = user.fullname;
    ...
    if (!staffName) {
        staffName = prompt('Enter your full name to see assigned tasks:');
        if (staffName) {
...
function addSearchBar() {
    ...
    searchDiv.innerHTML = '<input type="text" id="searchInput" placeholder="🔍 Search assigned requests...">';
...
function exportToPDF() {
    ...
    let html = `<html><head><title>My Assigned Requests</title></head><body>
    ...
    link.download = `my-assigned-requests-${new Date().toISOString().split('T')[0]}.pdf`;
    link.click();
    alert('PDF exported!');
}
    ...
    if (calendarActive) {
        if (!calendarDiv) {
            ...
            for (let d = 1; d <= lastDay.getDate(); d++) {
                ...
                calHtml += `<div class="calendar-day ${hasRequests ? 'has-request' : ''}" onclick="alert('${dateStr}: ${hasRequests || 0} assigned requests')">${d}</div>`;
            }
            calHtml += '</div></div>';
            const container = document.getElementById('requestsContainer');
...
function renderRequests() {
    ...
    if (!staffName) {
        if (container) {
            container.innerHTML = '<div class="no-requests"><i class="fas fa-exclamation-triangle"></i> Please enter your staff name to see assigned requests.</div>';
        }
        return;
    }
    ...
    if (searchTerm) {
        ...
        myRequests = myRequests.filter(r => (r.title && r.title.toLowerCase().includes(term)) || 
    ...
    if (myRequests.length === 0) {
        container.innerHTML = '<div class="no-requests"><i class="fas fa-inbox"></i> No requests assigned to you yet.</div>';
        return;
    }
    ...
    [...myRequests].reverse().forEach(req => {
        ...
        if (req.priority === 'High') priorityIcon = '<i class="fas fa-arrow-up"></i>';
        if (req.priority === 'Low') priorityIcon = '<i class="fas fa-arrow-down"></i>';
        
        let statusIcon = '<i class="fas fa-hourglass-half"></i>';
        if (req.status === 'inprogress') statusIcon = '<i class="fas fa-spinner fa-pulse"></i>';
        if (req.status === 'completed') statusIcon = '<i class="fas fa-check-circle"></i>';
        
        const card = document.createElement('div');
        card.className = 'request-card';
        card.innerHTML = `
            <h3><i class="fas fa-wrench"></i> ${escapeHtml(req.title)} <span style="background:${priorityColor}; color:white; padding:3px 12px; border-radius:20px; font-size:10px;">${priorityIcon} ${req.priority || 'Medium'}</span></h3>
            <p><i class="fas fa-door-open"></i> <strong>Room:</strong> ${escapeHtml(req.room)}</p>
            <p><i class="fas fa-user-graduate"></i> <strong>Student:</strong> ${escapeHtml(req.studentName || req.studentNumber)}</p>
            ...
            ${req.assignedDate ? `<p><i class="fas fa-clock"></i> <strong>Assigned on:</strong> ${req.assignedDate}</p>` : ''}
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    (function displayStaffName() {
        ...
        const staffName = getCurrentStaffName();
        if (staffName && welcomeDiv) {
            welcomeDiv.innerHTML = '<i class="fas fa-hand-peace"></i> Welcome back, <strong style="color: #af954c;">' + escapeHtml(staffName) + '</strong>! You can only see requests assigned to you.';
        } else if (welcomeDiv) {
